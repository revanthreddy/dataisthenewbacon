...
NationalInstruments.HtmlVI.UIActivityService = (function() {
    ...
    function createUIActivityObject(params) {
        ...
        if(params.element === undefined || (params.element instanceof Element) === false) {
            errorMessage = NationalInstruments.HtmlVI.replaceStrings(NationalInstruments.HtmlVI.errorMessages.PARAMETER_NOT_DOM_ELEMENT, params.element);
            throw new Error(errorMessage);
        }
        if(params.id === undefined || ((typeof params.id) === 'string') === false) {
            errorMessage = NationalInstruments.HtmlVI.replaceStrings(NationalInstruments.HtmlVI.errorMessages.PARAMETER_NOT_STRING, params.id);
            throw new Error(errorMessage);
        }
        if(params.up === undefined && params.down === undefined && params.move === undefined && params.cancelled === undefined && params.unregistered === undefined) {
            errorMessage = NationalInstruments.HtmlVI.replaceStrings(NationalInstruments.HtmlVI.errorMessages.REQUIRES_AT_LEAST_ONE_PARAMETER, 'up, down, move, cacelled, unregistered', params);
            throw new Error(errorMessage);
        }
    ...
    function upAction(activity, evtargs) {
        ...
        if(activity.id === currentActivityId) {
            ...
            errorMessage = NationalInstruments.HtmlVI.replaceStrings(NationalInstruments.HtmlVI.errorMessages.UNEXPECTED_BEHAVIOR, 'Attempted to complete activity that was NOT the current activity. Only the current activity should be able to fire the up action.');
            throw new Error(errorMessage);
        }
    }
    ...
    function register(params) {
        ...
        if (params === undefined || params === null) {
            errorMessage = NationalInstruments.HtmlVI.replaceStrings(NationalInstruments.HtmlVI.errorMessages.NO_PARAMETERS_SPECIFIED, 'NationalInstruments.HtmlVI.UIActivityService.register');
            throw new Error(errorMessage);
        }
    ...
    function unregister(id) {
        ...
        if (id === undefined || activity === undefined || activity === null) {
            errorMessage = NationalInstruments.HtmlVI.replaceStrings(NationalInstruments.HtmlVI.errorMessages.UNKNOWN_ID, id, 'Make sure to register an activity with the UIActivityService before attempting to remove the activity.');
            throw new Error(errorMessage);
        }
